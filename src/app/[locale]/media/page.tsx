...
const items = [
    ...
    },
    {
        ...
        href: 'up-in-the-air',
        type: '书籍',
        cover: 'https://chemingqiang.oss-cn-shenzhen.aliyuncs.com/img/t6_book_aiqiyi_2298665391711535259.jpg',
        year: 2009,
        desc: '作者：心梦无痕',
        rating: 7.8,
        tags: ['小说', '仙侠'],
...
export default function BookMovieGallery() {
    ...
    const filtered = filter === '全部' ? items : items.filter(item => item.type === filter);

    return (
        <section className="relative z-10 bg-white min-h-screen flex flex-col items-center py-20 px-6">
            {/* 背景视频 */}
            <VideoBackground videoSrc="https://chemingqiang.oss-cn-shenzhen.aliyuncs.com/img/mov_1750092393646.mov" opacity={0.25} />
                ...
                {categories.map(cat => (
                    <button
                        ...
                        className={`
                            ...
                            ${filter === cat ? 'scale-110 ring-4 ring-cyan-400 shadow-cyan-400' : 'opacity-80 hover:opacity-100 hover:scale-105'}
                            select-none
                            backdrop-blur-sm
                        `}
                        style={{ letterSpacing: '0.08em', fontFamily: "'JetBrains Mono', monospace" }}
                    >
                        {cat}
                    </button>
                ))}
                ...
                {filtered.map(item => (
                    ...
                    >
                        ...
                        <div
                            ...
                            "
                        >
                            <h3 className="text-xl font-bold mb-1 tracking-wide drop-shadow-lg">{item.title}</h3>
                            <div className="text-sm mb-2 opacity-80">{item.type} · {item.year}</div>
                            <p className="text-sm line-clamp-3">{item.desc}</p>
                            <div className="flex flex-wrap gap-2 mt-3 items-center">
                                {item.rating && (
                                    <span className="text-yellow-400 font-semibold text-sm drop-shadow-lg">★ {item.rating}</span>
                                )}
                                {item.tags?.map(tag => (
                                    <span key={tag} className="bg-cyan-900/70 rounded-full px-3 py-1 text-xs font-medium tracking-wider drop-shadow-lg">
                                        {tag}
                                    </span>
                                ))}
                            </div>
