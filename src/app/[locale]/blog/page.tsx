...
export default async function BlogPage({ params }: BlogPageProps) {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900  pt-32">
      <section className="max-w-7xl mx-auto px-4 py-12">
                ...
                <section className="relative h-48 overflow-hidden">
                  {post.cover ? (
                    <section className="relative w-full h-full">
                      ...
                      />
                      {/* 渐变遮罩 */}
                      <section className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent" />

                      {/* 标题叠加在图片上 */}
                      <section className="absolute bottom-0 left-0 right-0 p-6">
                        <h2 className="text-white text-xl font-bold line-clamp-2 mb-2 group-hover:text-blue-300 transition-colors">
                          {post.title}
                        </h2>

                        {/* 作者信息 */}
                        <section className="flex items-center text-slate-300 text-sm">
                    ...
                    <section className="w-full h-full bg-gradient-to-br from-blue-600 to-purple-700 flex items-center justify-center relative">
                      <section className="text-center text-white">
                        <section className="text-4xl mb-2">📝</section>
                      ...
                      <section className="absolute bottom-0 left-0 right-0 p-6">
                        ...
                        <section className="flex items-center text-slate-200 text-sm">
                          <section className="w-6 h-6 bg-white/20 rounded-full flex items-center justify-center mr-2">
        ...
        {posts.length === 0 && (
          <section className="text-center py-20">
            <section className="text-8xl mb-6">📝</section>
            <section className="text-slate-400 text-xl mb-4">
        ...
        {/* {process.env.NODE_ENV === 'development' && (
          <section className="mt-12 p-6 bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50">
            <h3 className="text-white font-bold mb-4">🔧 调试信息</h3>
            <section className="text-slate-300 space-y-2">
              <p>
